...
point camera_pos(4, 4, 4);
point camera_target(0, 0, 0);
point camera_up(0, 0, 1);
point camera_right = camera_up ^ (camera_target - camera_pos);
point camera_left;

void set_camera() {
    point up(0, 0, 1);
    point dir = camera_target - camera_pos;
    dir.normalize();
    camera_left = dir;
    camera_right = up ^ dir;
    camera_right.normalize();
    camera_up = dir ^ camera_right;
}
...
void drawFloor() {
    for(int i = 0; i < 100; i++) {
        for(int j = 0; j < 100; j++) {
            ...
            glBegin(GL_QUADS);
            {
                glVertex3f(-50 + i * 1, -50 + j * 1, 0);
                glVertex3f(-50 + (i + 1) * 1, -50 + j * 1, 0);
                glVertex3f(-50 + (i + 1) * 1, -50 + (j + 1) * 1, 0);
                glVertex3f(-50 + i * 1, -50 + (j + 1) * 1, 0);
...
void display(){
	...
	********************/
    ...
    gluLookAt(camera_pos.x, camera_pos.y, camera_pos.z, camera_pos.x + camera_left.x, camera_pos.y + camera_left.y, camera_pos.z + camera_left.z, camera_up.x, camera_up.y, camera_up.z);
